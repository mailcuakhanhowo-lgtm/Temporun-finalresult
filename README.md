...
- **Giới hạn của Phương án Gộp Vector Sớm (Early Weighted Fusion)**: Đội thi đã thử nghiệm việc dùng LLM bóc tách câu truy vấn thành các đặc trưng nguyên tử (Câu gốc, Chủ thể, Hành động, Bối cảnh), nhúng chúng thành các vector độc lập. Sau đó, **nhân các vector này với hệ số trọng lượng (`FUSION_WEIGHTS`) và cộng gộp chúng lại thành một siêu-vector duy nhất (Gộp Vector Tuyến Tính)** trước khi tính toán độ tương đồng (Dot-product). Tuy nhiên, thực tế nghiệm thu cho thấy phương án này **không thể nâng cao điểm số tổng thể**. Nguyên nhân là do trong không gian vector đa chiều của CLIP, việc cộng gộp tuyến tính các vector đặc trưng rời rạc làm lệch hướng siêu-vector khỏi "Mỏ neo" ngữ nghĩa tổng thể (Holistic Context), dẫn đến hiện tượng pha loãng và nhiễu loạn. Việc tìm kiếm trực tiếp bằng một câu nguyên bản đầy đủ ngữ cảnh vẫn mang lại độ chính xác (Precision) cao nhất.
